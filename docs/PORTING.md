...
Record discoveries here as they happen (surprising `#ifdef` gaps, API quirks,
 ...
 - 2026-09-06 — **M5: FreeBSD build fix — Terminal uses util.h for forkpty on BSDs (patch 0033).**
   Terminal/main.cpp includes `<pty.h>` for `forkpty()`, but that header is glibc/Serenity-only; FreeBSD provides
   `forkpty()`/`openpty()` in `<util.h>` (compatible signature). Patch 0033 gates the include to Serenity/glibc and
   uses `<util.h>` on other hosts. It's the only built file including `<pty.h>`. Linux/Serenity unchanged (verified
   locally, 250/250; m4-launch-terminal passes).
